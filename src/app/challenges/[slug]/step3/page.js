...
function ChallengeStep3Page() {
  ...
  // Maximum *input* file size (before processing). Prevents absurd camera exports.
  const MAX_INPUT_BYTES = 20 * 1024 * 1024 // 20 MB
  // Marketing "original" should not exceed this after resizing (should be rare).
  const MAX_MARKETING_BYTES = 3 * 1024 * 1024 // 3 MB
  // Marketing "original" resize settings:
        ...
        steps: [
          {
            ...
            videoUrl: '/demo/video/step_3.mp4',
            referenceImageUrl: '/demo/images/step3_reference.jpeg',
          },
        ],
      })
  ...
  // -------- Load last Step 3 image for this challenge --------
  useEffect(() => {
    if (!user || adminDemo || demo || !challengeId) return

    let cancelled = false

    const loadLastStep3Image = async () => {
      const { data, error } = await supabase
        .from('uploads')
        .select('image_url')
        .eq('user_id', user.id)
      ...
      if (error) {
        console.error('Error loading existing Step 3 image:', error.message)
        return
      }
  ...
  // -------- Derive Step 3 assets from challenge JSON --------
  const stepConfig = (() => {
    if (!challenge || !challenge.steps) return null
    const steps = Array.isArray(challenge.steps) ? challenge.steps : []
    if (!steps.length) return null

    // ✅ stepNumber can be stored as a string in JSONB ("3"), so normalize.
    const byNumber = steps.find((s) => Number(s.stepNumber) === 3)
    return byNumber || steps[2] || steps[steps.length - 1] || steps[0]
  })()
  ...
  const referenceImageUrlRaw =
    stepConfig?.referenceImageUrl || '/style_one/step3_reference.jpeg'
  ...
  const demoYourImageUrl = '/demo/images/stylist_step3_reference.jpeg'

  // ✅ DEMO video placeholder image
  const demoVideoPlaceholderUrl = '/demo/images/video_placeholder_step3.jpeg'
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      // Only if we have a raw file in memory (normal path). If not, skip.
      let originalPath = null

      if (rawSelectedUrl && rawSelectedFile) {
        // Generate a capped "marketing original"
        ...
        // Put originals under a dedicated folder
        originalPath = makeUploadPath(userId, 'originals/step3', originalFile)
      ...
      // 2) Upload cropped/judging image (your existing behaviour)
      const filePath = makeUploadPath(userId, 'step3', file)

      const { data, error } = await supabase.storage.from('uploads').upload(filePath, file)

      if (error) {
        console.error('❌ Storage upload failed (step3 cropped):', error.message)
      ...
      if (!adminDemo && user && challengeId) {
        const payload = {
          user_id: user.id,
          step_number: 3,
          image_url: croppedPath, // judged/portfolio
          challenge_id: challengeId,
          original_image_url: originalPath, // marketing "original" (downsized)
        }
  ...
  const proceedToFinished = () => {
    ...
    router.push('/challenges/' + slug + '/finished' + suffix)
  }

  // Combined loading state
  if (loadingUser || loadingChallenge || !slug) {
    return <p>Loading challenge step 3…</p>
  }
  ...
  // --- Demo placeholder container formatting (same pattern as Step 2 / landing) ---
  ...
  return (
    ...
    >
      ...
      <h1 style={{ marginBottom: '0.5rem' }}>Step 3: Polish & Detail</h1>
      <hr
        style={{
          width: '50%',
      ...
      <p style={{ marginBottom: '0.75rem', fontSize: '1rem', color: '#ddd' }}>
        This is where you refine the details and make the look camera-ready.
      </p>
      ...
      >
        <li>Watch Patrick’s demo for Step 3.</li>
        <li>Smooth, polish, and perfect the finish so it looks clean from every angle.</li>
        <li>Take your photo in the same framing as Steps 1 and 2 for a consistent portfolio.</li>
      </ol>
          ...
          <div style={placeholderFrame}>
            <div style={placeholderInner}>
              <Image
                ...
                alt="Demo video placeholder - Step 3"
                width={1200}
                height={675}
      ...
      >
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          <div style={overlayFrame}>
            <img src={referenceImageUrl} alt="Patrick Step 3" style={previewImageStyle} />
          </div>
        </div>
      ...
      {!showOptions && !adminDemo && !demo && (
        <form onSubmit={handleUpload} style={{ marginTop: '2rem' }}>
          ...
          >
            Make sure the hairstyle fills the frame in <strong>portrait</strong> mode, with the head and hair inside the
            oval.
          </p>
          ...
          >
            {uploading ? 'Uploading…' : '✅ Confirm, Add to Portfolio & View Finished Look'}
          </button>
      ...
      {(showOptions || adminDemo || demo) && (
        ...
        >
          ...
          <p style={{ fontSize: '1.1rem', color: '#fff', lineHeight: 1.5, marginBottom: '1rem' }}>
            You’ve completed all three steps. Next, confirm your finished look and build your portfolio.
          </p>

          <button
            onClick={proceedToFinished}
            disabled={navigating}
            style={{
          ...
          >
            {navigating ? 'Loading finished look…' : '✅ Go to Finished Look'}
          </button>
          ...
          {navigating && (
            <p style={{ marginTop: '0.75rem', fontSize: '0.95rem', color: '#cce8d5' }}>
              Moving to your Finished Look… please wait.
            </p>
          )}
        </div>
      )}
