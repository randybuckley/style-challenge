...
function FinishedLookInner() {
  ...
  // -------- Session + admin flag (same as v6 Finished) ----------
  useEffect(() => {
    let alive = true
  ...
  // -------- Fetch any existing step-4 uploads (v6 behaviour) ----------
  useEffect(() => {
    const fetchFinishedImages = async () => {
      ...
      if (data && data.length > 0) {
        const mannequin = data.find((img) =>
          img.image_url.includes('finished-mannequin')
        )
        const model = data.find((img) =>
          img.image_url.includes('finished-model')
        )
  ...
  // -------- Upload handler (unchanged from v6, plus admin_demo) ----------
  ...
  // Keep existing portfolio route for now, but add a navigation ticker
  ...
  // -------- Shared styles (same as v6, but clean oval only) ----------
  const overlayFrame = {
    position: 'relative',
    width: '100%',
    maxWidth: 320,
  ...
  return (
    ...
    >
      ...
      <h1 style={{ marginBottom: '0.5rem' }}>
        Finished Look: Show Off Your Best Work
      </h1>
      <hr
        style={{
          width: '50%',
      ...
      >
        ...
        <br />
        A clear mannequin photo completes the challenge, and an optional
        in-real-life model photo turns your style into a powerful portfolio
        image.
      </p>
      ...
      {(showOptions || adminDemo) && (
        ...
        >
          ...
          >
            Does this final look represent your <strong>best work</strong>? If
            yes, you’re ready to view your portfolio.
          </p>
          ...
          {navigating && (
            ...
            >
              Loading your portfolio… please wait.
            </p>
          )}
        </div>
      )}
