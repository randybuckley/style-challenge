...
function ChallengeStep2Inner() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      // ✅ Insert row (non-demo only) — keep starter schema unchanged
      if (!adminDemo && user) {
  ...
  // ---- shared “Your Version” frame styles ----
  const overlayFrame = {
    position: 'relative',
    width: '100%',
    maxWidth: 320,
  ...
  return (
    ...
    >
      ...
      <p style={{ marginBottom: '0.75rem', fontSize: '1rem', color: '#ddd' }}>Before you take your Step&nbsp;2 photo:</p>
      ...
      <p style={{ marginBottom: '2rem', fontSize: '1rem', color: '#ddd' }}>
        <strong>Important:</strong> Keep your phone <strong>upright (portrait)</strong> and make sure the Step&nbsp;2 detail is clearly visible inside the oval.
      </p>
      ...
      {(showOptions || adminDemo) && (
        <div style={{ marginTop: '3rem', padding: '1.5rem', border: '2px solid #28a745', borderRadius: '8px', background: 'rgba(40, 167, 69, 0.1)', textAlign: 'center' }}>
          <h2 style={{ color: '#28a745', fontSize: '1.5rem', marginBottom: '0.75rem', fontWeight: '700' }}>🎉 Step 2 looks great!</h2>
